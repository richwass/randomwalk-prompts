...
**3. Choose this quarter's Key Results.** For each objective:
...
- For each KR: "What's the current state, and what's the target? Say: 'Improve ___ from X to Y, an improvement of Z%.'"
- KRs should be specific enough that a stranger could verify they were done. "All P0 alerts have runbooks by March 1" is verifiable. "Improve reliability" is not.
...
**5. Effort and feasibility.**
- "Rough t-shirt size (S/M/L/XL) for each KR — so we can smell-test whether this fits in a quarter."
- "These effort estimates are for feasibility checking, not for filling your capacity."
...
**Do not produce output until at least 4-5 rounds of follow-up questions.**
...
OBJECTIVE 1: [Durable mandate], as measured by [observable state or long-running target]
  ...
  KR 1.1: Improve [metric/state] from [current] to [target], an improvement of [X%].
...
**Next quarter:** Return to this stage. Keep the Objectives. Replace the KRs with the next quarter's best bets.
